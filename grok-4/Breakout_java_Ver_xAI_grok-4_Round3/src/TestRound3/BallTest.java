...
public class BallTest {
    ...
    void testReverseDy() {
        ...
        ball.reverseDy();
        ball.move();
        assertEquals(62, ball.getY());
    }

    @Test
    // ทดสอบ intersects: กรณีชน (true) และไม่ชน (false) เพื่อครอบคลุม branch ทั้งหมดในเงื่อนไข &&
    void testIntersects() {
        Ball ball = new Ball(50, 50, 20, 20, 0, 0);
        assertTrue(ball.intersects(60, 60, 10, 10)); // ชน
        assertFalse(ball.intersects(0, 0, 10, 10)); // ไม่ชน (branch false ใน conditions)
        assertFalse(ball.intersects(100, 100, 10, 10)); // ไม่ชน
    }
}